...
class IVectorStore(ABC):
    ...
    @abstractmethod
    def get_node_emb(self, node_id: str,
                     namespace: Optional[str] = None) -> Optional[np.ndarray]:
        """Get node embedding."""
        pass

    @abstractmethod
    def query_similar_nodes(self,
                            query_embedding: np.ndarray,
                            top_k: int = 5,
                            namespace: Optional[str] = None) -> List[Tuple[str,
                                                                           float]]:
        """Query similar nodes."""
        pass
...
class IEmbService(ABC):
    ...
    @abstractmethod
    def insert_node_emb(self, node_id: str, node_text: str,
                        namespace: Optional[str] = None) -> None:
        """Insert node embedding from text."""
        pass

    @abstractmethod
    def update_node_emb(self, node_id: str, node_text: str,
                        namespace: Optional[str] = None) -> None:
...
class PineconeVectorStore(IVectorStore):
    ...
    def _ensure_index_exists(
        ...
        try:
            ...
            raise IndexError(
                f"Failed to create/access index {self.index_name}: {e}")
    ...
    def get_node_emb(self, node_id: str,
        ...
        try:
            ...
            if response.vectors.get(node_id):
                return np.array(
                    response.vectors[node_id].values,
                    dtype=np.float32)
            return None
        except Exception as e:
...
class FAISSVectorStore(IVectorStore):
    ...
    def __init__(self,
        ...
        if not FAISS_AVAILABLE:
            raise ImportError(
                "FAISS package not available. Install with: pip install faiss-cpu")
    ...
    def load_index(self) -> None:
        ...
        try:
            index_path = Path(self.file_store.prefix_path) / \
            ...
            if index_path.exists():
                ...
                for namespace, namespace_data in data.items():
                    ...
                    vectors = np.array(
                        namespace_data["vectors"], dtype=np.float32)
                    if vectors.shape[1] != self.dimension:
                        raise ValueError(
                            f"Dimension mismatch in namespace {namespace}")
                    self.namespaces[namespace]["index"].add(vectors)

                self.logger.info(
                    f"Loaded index with {len(self.namespaces)} namespaces")
            else:
                self._create_default_namespace()
        except Exception as e:
            self.logger.warning(
                f"Failed to load index: {e}, creating new index")
            self._create_default_namespace()
    ...
    def save_index(self) -> None:
        ...
        try:
            ...
            for namespace, namespace_data in self.namespaces.items():
                vectors = namespace_data["index"].reconstruct_n(
                    0, namespace_data["index"].ntotal)
                data[namespace] = {
                    "id_to_index": namespace_data["id_to_index"],
        ...
        try:
            ...
            if node_id not in namespace_data["id_to_index"]:
                raise NamespaceError(
                    f"Node {node_id} not found in namespace {namespace}")
        ...
        try:
            ...
            for score, idx in zip(scores[0], indices[0]):
                if idx >= 0:  # Valid index
                    try:
                        node_id = next(
                            key for key, value in namespace_data["id_to_index"].items() if value == idx)
                        similar_nodes.append((node_id, float(score)))
                    except StopIteration:
                        self.logger.warning(
                            f"No node found for index {idx} in namespace {namespace}")
...
class EmbStore(IEmbService):
    ...
    def __init__(self,
        ...
        self.logger = logger or logging.getLogger(__name__)
        self.logger.warning(
            "EmbStore is deprecated. Use EmbStoreService with dependency injection.")

        self.embedding_provider = OpenAIEmbeddingProvider(
            emb_api_key, emb_model, logger)
    ...
    def query_similar_nodes(self,
        ...
        return self.vector_store.query_similar_nodes(
...
class EmbStoreLocal(IEmbService):
    ...
    def __init__(self,
        ...
        self.logger = logger or logging.getLogger(__name__)
        self.logger.warning(
            "EmbStoreLocal is deprecated. Use EmbStoreService with dependency injection.")

        self.embedding_provider = OpenAIEmbeddingProvider(
            emb_api_key, emb_model, logger)
        self.vector_store = FAISSVectorStore(
            emb_dim, file_store, index_file_name, logger)
...
class EmbStoreService(IEmbService):
    ...
    def query_similar_nodes(self,
                            ...
                            namespace: Optional[str] = None) -> List[Tuple[str,
                                                                           float]]:
        """Query similar nodes by text."""
        query_embedding = self.embedding_provider.generate_embedding(
            query_text)
        return self.vector_store.query_similar_nodes(
            query_embedding, top_k, namespace)
...
class EmbServiceFactory:
    ...
    @staticmethod
    def create_pinecone_store(emb_api_key: str,
                              pinecone_api_key: str,
                              index_name: str,
                              emb_model: str,
                              emb_dim: int,
    ...
    @staticmethod
    def create_local_store(emb_api_key: str,
                           file_store: 'IFileStore',
                           emb_model: str,
                           emb_dim: int,
                           index_file_name: str = "emb_index.json",
    ...
    @staticmethod
    def create_custom_store(embedding_provider: IEmbeddingProvider,
                            vector_store: IVectorStore,
                            logger: Optional[logging.Logger] = None) -> EmbStoreService:
