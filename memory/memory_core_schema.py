...
class MemoryCoreValidator(IMemoryCoreValidator):
    ...
    def validate_structure(self, memory_core_path: str) -> List[str]:
        ...
        if not core_path.exists():
            errors.append(
                f"Memory core path does not exist: {memory_core_path}")
            return errors

        if not core_path.is_dir():
            errors.append(
                f"Memory core path is not a directory: {memory_core_path}")
            return errors
        ...
        if not config_file.exists():
            errors.append(
                f"Required config file missing: {self.schema.CONFIG_FILE}")
        elif not config_file.is_file():
            errors.append(
                f"Config path is not a file: {self.schema.CONFIG_FILE}")

        # Check required assets directory
        assets_dir = core_path / self.schema.ASSETS_DIR
        if not assets_dir.exists():
            errors.append(
                f"Required assets directory missing: {self.schema.ASSETS_DIR}")
        elif not assets_dir.is_dir():
            errors.append(
                f"Assets path is not a directory: {self.schema.ASSETS_DIR}")
        else:
            # Check required asset subdirectories
            img_dir = assets_dir / self.schema.IMG_DIR
            if not img_dir.exists():
                errors.append(
                    f"Required img directory missing: {self.schema.ASSETS_DIR}/{self.schema.IMG_DIR}")
            elif not img_dir.is_dir():
                errors.append(
                    f"Img path is not a directory: {self.schema.ASSETS_DIR}/{self.schema.IMG_DIR}")

            voice_dir = assets_dir / self.schema.VOICE_DIR
            if not voice_dir.exists():
                errors.append(
                    f"Required voice directory missing: {self.schema.ASSETS_DIR}/{self.schema.VOICE_DIR}")
            elif not voice_dir.is_dir():
                errors.append(
                    f"Voice path is not a directory: {self.schema.ASSETS_DIR}/{self.schema.VOICE_DIR}")
        ...
        if not data_dir.exists():
            ...
            errors.append(
                f"Data path is not a directory: {self.schema.DATA_DIR}")
        ...
        if "embedding" in config:
            ...
            for field in required_embedding_fields:
                if field not in embedding_config:
                    errors.append(
                        f"Required embedding config field missing: {field}")
            ...
            if provider == "remote":
                ...
                for field in remote_fields:
                    if field not in embedding_config:
                        errors.append(
                            f"Required remote embedding config field missing: {field}")
            elif provider == "local":
                # Local provider is valid with just the basic fields
                pass
            elif provider is not None:
                errors.append(
                    f"Invalid embedding provider: {provider}. Must be 'local' or 'remote'")
...
class MemoryCoreInitializer:
    ...
    def create_memory_core_structure(self, memory_core_path: str) -> None:
        ...
        if core_path.exists() and not core_path.is_dir():
            raise ValueError(
                f"Path exists as file, not directory: {memory_core_path}")
        ...
        self.logger.info(
            f"Created memory core structure at: {memory_core_path}")
    ...
    def create_default_config(
        ...
        if provider == "remote":
            if not pinecone_api_key or not pinecone_index_name:
                raise ValueError(
                    "Remote provider requires pinecone_api_key and pinecone_index_name")
        ...
        if validate:
            ...
            if errors:
                raise ValidationError(
                    f"Memory core validation failed: {'; '.join(errors)}")

        self.logger.info(
            f"Successfully initialized memory core at: {memory_core_path}")
...
def load_memory_core_config(memory_core_path: str) -> Dict[str, Any]:
    ...
    if config_errors:
        raise ValidationError(
            f"Memory core config validation failed: {'; '.join(config_errors)}")
