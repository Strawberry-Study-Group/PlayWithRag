...
class IGraphStore(ABC):
    ...
    @abstractmethod
    def get_edge(self, source_node_id: str,
                 target_node_id: str) -> Optional[Dict[str, Any]]:
        """Get an edge by source and target node IDs."""
        pass

    @abstractmethod
    def get_neighbor_info(
            self, node_id: str, hop: int = 1) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
...
class SchemaValidator(IGraphValidator):
    ...
    def _load_schema(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise SchemaError(
                f"Failed to load schema from {self.schema_file}: {e}")

    def _validate_required_fields(
            self, data: Dict[str, Any], required_fields: Dict[str, Any]) -> bool:
    ...
    def _validate_type(self, value: Any, expected_type: str,
        ...
        if 'constraints' in field_spec:
            ...
            if expected_type == 'string' and isinstance(value, str):
                if 'min_length' in constraints and len(
                        value) < constraints['min_length']:
                    return False
                if 'max_length' in constraints and len(
    ...
    def _apply_defaults(
            self, data: Dict[str, Any], required_fields: Dict[str, Any]) -> Dict[str, Any]:
...
class GraphStore(IGraphStore):
    def __init__(self,
                 ...
                 validator: Optional[IGraphValidator] = None,
            ...
            if self.file_store.file_exists(self.graph_file_name):
                ...
                if self.validator:
                    try:
                        ...
                        self.logger.warning(
                            f"Loaded graph failed validation: {e}")

                self.logger.info(
                    f"Loaded graph with {len(graph.get('node_dict', {}))} nodes")
            else:
    ...
    def parse_edge_key(
            ...
            source_node_id: str,
            target_node_id: str,
            is_directed: bool = False) -> str:
        """Generate edge key for storage."""
        if is_directed:
            return f"{source_node_id}->{target_node_id}"
        else:
            keys = sorted([source_node_id, target_node_id])
    ...
    def add_node(self, node: Dict[str, Any]) -> None:
        ...
        if node_name in self.graph["node_name_to_id"]:
            raise DuplicateError(
                f"Node with name '{node_name}' already exists")
    ...
    def update_node(self, node: Dict[str, Any]) -> None:
        ...
        if old_name != new_name:
            if new_name in self.graph["node_name_to_id"]:
                raise DuplicateError(
                    f"Node with name '{new_name}' already exists")
    ...
    def delete_edge(self, source_node_id: str, target_node_id: str) -> None:
        ...
        edge_key_directed = self.parse_edge_key(
            source_node_id, target_node_id, True)
        edge_key_undirected = self.parse_edge_key(
            source_node_id, target_node_id, False)
    ...
    def update_edge(self, edge: Dict[str, Any]) -> None:
        ...
        if not source_id or not target_id:
            raise ValidationError(
                "Source and target node IDs are required for edge update")
        ...
        if not existing_edge:
            raise EdgeNotFoundError(
                f"Edge between '{source_id}' and '{target_id}' not found")
    ...
    def get_neighbor_info(
        ...
        if hop == 1:
            ...
            node_list = [self.graph["node_dict"][neighbor_id]
                         for neighbor_id in neighbor_ids if neighbor_id in self.graph["node_dict"]]
            ...
            for _ in range(hop):
                ...
                for node in queue:
                    if node not in visited:
                        visited.add(node)
                        neighbor_ids = self.graph["neighbor_dict"].get(
                            node, [])
                        for neighbor_id in neighbor_ids:
                            if neighbor_id not in visited and neighbor_id in self.graph["node_dict"]:
                                node_list.append(
                                    self.graph["node_dict"][neighbor_id])
...
class GraphStoreFactory:
    ...
    @staticmethod
    def create_local_store(file_store: IFileStore,
    ...
    def create_cloud_store(provider: str, **config) -> IGraphStore:
        ...
        raise NotImplementedError(
            f"Cloud graph store provider '{provider}' not yet implemented")
