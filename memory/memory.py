...
class IMemoryCore(ABC):
    ...
    def add_concept(self,
                    concept_name: str,
                    concept_type: str,
                    concept_attributes: Dict[str,
                                             Any],
                    is_editable: bool = True,
                    image_path: Optional[str] = None,
                    refs: Optional[Dict[str,
                                        List[str]]] = None) -> str:
    ...
    def update_concept(self,
                       ...
                       concept_attributes: Optional[Dict[str,
                                                         Any]] = None,
                       image_path: Optional[str] = None,
                       refs: Optional[Dict[str,
                                           List[str]]] = None) -> None:
    ...
    def query_similar_concepts(
            self, query_text: str, top_k: int = 5) -> List[Tuple[Dict[str, Any], float]]:
    ...
    def get_related_concepts(self,
                             ...
                             concept_type: Optional[str] = None) -> Tuple[List[Dict[str,
                                                                                    Any]],
                                                                          List[Dict[str,
                                                                                    Any]]]:
...
class MemoryCoreService(IMemoryCore):
    ...
    def add_concept(self,
        ...
        builder.with_attributes(
            concept_attributes).with_editability(is_editable)
        ...
        self.emb_store.insert_node_emb(
            concept_id,
            node_text,
            namespace=ConceptGraphConstants.NAMESPACE_FULL_NODE)

        node_name_embedding_id = get_node_name_embedding_id(concept_id)
        self.emb_store.insert_node_emb(
            node_name_embedding_id,
            concept_name,
            namespace=ConceptGraphConstants.NAMESPACE_NODE_NAME)
    ...
    def delete_concept(self, concept_id: str) -> None:
        ...
        if image_path:
            try:
                ...
                self.logger.warning(
                    f"Failed to delete image file for concept {concept_id}: {e}")

        # Delete embeddings
        self.emb_store.delete_node_emb(
            concept_id, namespace=ConceptGraphConstants.NAMESPACE_FULL_NODE)
    ...
    def update_concept(self,
        ...
        if concept_name:
            ...
            node_name_embedding_id = get_node_name_embedding_id(concept_id)
            self.emb_store.update_node_emb(
                node_name_embedding_id,
                concept_name,
                namespace=ConceptGraphConstants.NAMESPACE_NODE_NAME)
        ...
        if concept_attributes:
            node[ConceptGraphConstants.FIELD_NODE_ATTRIBUTES].update(
                concept_attributes)
        ...
        if not ConceptOperations.validate_concept(node, self.logger):
            ...
            raise ValueError(
                f"Updated concept validation failed: {', '.join(errors)}")
    ...
    def add_relation(self, source_concept_id: str, target_concept_id: str,
        ...
        if not self.is_concept(source_concept_id):
            raise ConceptNotFoundError(
                f"Source concept '{source_concept_id}' not found")
        if not self.is_concept(target_concept_id):
            raise ConceptNotFoundError(
                f"Target concept '{target_concept_id}' not found")
        ...
        self.logger.debug(
            f"Added relation: {source_concept_id} -> {target_concept_id} ({relation_type})")
    ...
    def delete_relation(
        ...
        if not self.is_relation(source_concept_id, target_concept_id):
            raise RelationNotFoundError(
                f"Relation between '{source_concept_id}' and '{target_concept_id}' not found")

        self.graph_store.delete_edge(source_concept_id, target_concept_id)

        self.logger.debug(
            f"Deleted relation: {source_concept_id} -> {target_concept_id}")
    ...
    def update_relation(self, source_concept_id: str, target_concept_id: str,
        ...
        if relation_type:
            ...
            self.logger.debug(
                f"Updated relation: {source_concept_id} -> {target_concept_id} ({relation_type})")

    def get_relation(self, source_concept_id: str,
                     target_concept_id: str) -> Optional[Dict[str, Any]]:
    ...
    def query_similar_concepts(
            self, query_text: str, top_k: int = 5) -> List[Tuple[Dict[str, Any], float]]:
        """Query for similar concepts using embedding similarity."""
        similar_nodes = self.emb_store.query_similar_nodes(
            query_text, top_k, namespace=ConceptGraphConstants.NAMESPACE_FULL_NODE)
        similar_names = self.emb_store.query_similar_nodes(
            query_text, top_k, namespace=ConceptGraphConstants.NAMESPACE_NODE_NAME)
        ...
        for node_id_with_name, score in similar_names:
            node_id = node_id_with_name.split(
                ConceptGraphConstants.NODE_NAME_SEPARATOR)[0]
            concept = self.get_concept(node_id)
            if concept:
                similar_concepts.append((concept, score))
    ...
    def get_related_concepts(self,
        ...
        node_list, edge_list = self.graph_store.get_neighbor_info(
            concept_id, hop=hop)
        ...
        for edge in edge_list:
            ...
            source_node = self.graph_store.get_node(
                relation[ConceptGraphConstants.FIELD_SOURCE_NODE_ID])
            target_node = self.graph_store.get_node(
                relation[ConceptGraphConstants.FIELD_TARGET_NODE_ID])
    ...
    def search_concepts(
            self, criteria: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def validate_all_concepts(self) -> Dict[str, List[str]]:
        ...
        for node in all_nodes:
            ...
            if not ConceptOperations.validate_concept(node, self.logger):
                errors = ConceptOperations.get_validation_errors(
                    node, self.logger)
...
class MemoryCoreFactory:
    ...
    def create_from_memory_core(memory_core_path: str,
        ...
        # If config is provided, we still validate the structure but skip
        # config validation
        else:
            ...
            if structure_errors:
                ...
                raise ValidationError(
                    f"Memory core structure validation failed: {'; '.join(structure_errors)}")
        ...
        graph_file_name = files_config.get(
            "graph_file", schema.DEFAULT_GRAPH_FILE)
        index_file_name = files_config.get(
            "index_file", schema.DEFAULT_INDEX_FILE)

        # Create file store pointing to the assets/img directory for images
        from pathlib import Path
        assets_img_path = str(
            Path(memory_core_path) / schema.ASSETS_DIR / schema.IMG_DIR)
        ...
        if embedding_config["provider"] == "remote":
            ...
            raise ValueError(
                f"Invalid embedding store provider: {embedding_config['provider']}")

        return MemoryCoreService(file_store, graph_store, emb_store, logger)

    @staticmethod
    def create_custom(file_store: IFileStore,
                      graph_store: IGraphStore,
                      emb_store: IEmbService,
                      logger: Optional[logging.Logger] = None) -> MemoryCoreService:
