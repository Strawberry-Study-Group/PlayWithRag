...
class ConceptTextExtractor(IConceptVisitor):
    ...
    def visit_concept(self, concept: Dict[str, Any]) -> str:
        ...
        node_attributes = concept.get(ConceptGraphConstants.FIELD_NODE_ATTRIBUTES, {})
...
class ConceptValidator(IConceptVisitor):
    ...
    def visit_concept(self, concept: Dict[str, Any]) -> bool:
        ...
        if ConceptGraphConstants.FIELD_NODE_NAME in concept:
            if not isinstance(concept[ConceptGraphConstants.FIELD_NODE_NAME], str):
        ...
        if ConceptGraphConstants.FIELD_NODE_TYPE in concept:
            if not isinstance(concept[ConceptGraphConstants.FIELD_NODE_TYPE], str):
        ...
        if ConceptGraphConstants.FIELD_NODE_ATTRIBUTES in concept:
            if not isinstance(concept[ConceptGraphConstants.FIELD_NODE_ATTRIBUTES], dict):
                self.validation_errors.append("node_attributes must be a dictionary")
        
        if ConceptGraphConstants.FIELD_IS_EDITABLE in concept:
            if not isinstance(concept[ConceptGraphConstants.FIELD_IS_EDITABLE], bool):
        ...
        if ConceptGraphConstants.FIELD_REFS in concept:
            ...
            if not isinstance(refs, dict):
                ...
                for ref_type in expected_ref_types:
                    if ref_type in refs and not isinstance(refs[ref_type], list):
                        self.validation_errors.append(f"refs.{ref_type} must be a list")
        
        if self.validation_errors:
            self.logger.warning(f"Concept validation failed: {', '.join(self.validation_errors)}")
            return False
...
class ConceptStatisticsCollector(IConceptVisitor):
    ...
    def visit_concept(self, concept: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Count by type
        concept_type = concept.get(ConceptGraphConstants.FIELD_NODE_TYPE, "unknown")
        self.stats["concept_types"][concept_type] = self.stats["concept_types"].get(concept_type, 0) + 1
        ...
        attributes = concept.get(ConceptGraphConstants.FIELD_NODE_ATTRIBUTES, {})
    ...
    def _matches_criteria(self, concept: Dict[str, Any]) -> bool:
        ...
        for key, value in self.search_criteria.items():
            if key == ConceptGraphConstants.FIELD_NODE_TYPE:
                ...
                if isinstance(value, str) and value.lower() not in concept_name.lower():
                    return False
...
class ConceptOperations:
    ...
    @staticmethod
    def extract_text(concept: Dict[str, Any], include_refs: bool = False) -> str:
    ...
    @staticmethod
    def validate_concept(concept: Dict[str, Any], logger: Optional[logging.Logger] = None) -> bool:
        """Validate a concept."""
        validator = ConceptValidator(logger)
        return validator.visit_concept(concept)
    
    @staticmethod
    def get_validation_errors(concept: Dict[str, Any], logger: Optional[logging.Logger] = None) -> List[str]:
    ...
    @staticmethod
    def search_concepts(concepts: List[Dict[str, Any]], criteria: Dict[str, Any]) -> List[Dict[str, Any]]:
